    ...
    <div class="lg:col-span-2 space-y-6">
        <div>
            ...
            <input type="text" id="title" name="title" value="{{ old('title', $announcement->title ?? '') }}"
                class="w-full px-4 py-2 border border-blue-pale rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-medium @error('title') border-red-500 @enderror"
                required>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4"
                class="w-full px-4 py-2 border border-blue-pale rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-medium @error('description') border-red-500 @enderror"
                required>{{ old('description', $announcement->description ?? '') }}</textarea>
            @error('description')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>

        <div>
            <label class="block text-sm font-medium text-blue-medium mb-1">Gambar Pengumuman</label>
            <div id="file-drop-area"
                class="file-drop-area relative flex flex-col items-center justify-center w-full h-64 p-4 border-2 border-blue-pale border-dashed rounded-lg cursor-pointer transition-colors">
                <div id="preview-container"
                    class="absolute inset-0 p-2 @unless(isset($announcement) && $announcement->image_path) hidden @endunless">
                    <img id="image-preview"
                        src="{{ isset($announcement) ? asset('storage/' . $announcement->image_path) : '' }}"
                        class="w-full h-full object-contain" alt="Image Preview">
                </div>
                <div id="upload-prompt"
                    class="text-center @if(isset($announcement) && $announcement->image_path) opacity-0 hover:opacity-100 bg-black/50 transition-opacity flex flex-col items-center justify-center absolute inset-0 text-white @endif">
                    <svg class="w-10 h-10 mx-auto text-blue-light" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
    ...
    <div class="lg:col-span-1">
        ...
        <div class="flex items-center gap-4">
            ...
            <span id="status-text" class="font-semibold text-navy-dark">
                {{ old('is_active', $announcement->is_active ?? true) ? 'Aktif' : 'Nonaktif' }}
            </span>
        </div>
        <div class="mt-6">
            <label for="order" class="block text-sm font-medium text-blue-medium mb-1">Nomor Urut</label>
            <input type="number" name="order" id="order" value="{{ old('order', $announcement->order ?? 99) }}"
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (dropArea) {
            function showPreview(file) {
                ...
                reader.onload = function (e) { imagePreview.src = e.target.result; previewContainer.classList.remove('hidden'); uploadPrompt.classList.add('opacity-0', 'hover:opacity-100', 'bg-black/50', 'text-white'); }
            ...
            ['dragenter', 'dragover'].forEach(eventName => { dropArea.addEventListener(eventName, () => dropArea.classList.add('is-dragging'), false); });
            ['dragleave', 'drop'].forEach(eventName => { dropArea.addEventListener(eventName, () => dropArea.classList.remove('is-dragging'), false); });
