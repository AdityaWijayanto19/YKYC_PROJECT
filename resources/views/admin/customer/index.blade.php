                        ...
                        @forelse ($customers as $customer)
                            <tr class="border-b hover:bg-slate-50 transition-colors">
                                <td class="py-4 px-4 flex items-center gap-3">
                                    <img src="{{ $customer->avatar ? (Str::startsWith($customer->avatar, 'http') ? $customer->avatar : Storage::url($customer->avatar)) : Avatar::create($customer->name)->toBase64() }}"
                                        class="w-10 h-10 rounded-full object-cover" alt="{{ $customer->name }}">
                                    <div>
                                ...
                                <td class="px-4 text-center">
                                    <div class="flex justify-center items-center gap-2">
                                        <form action="{{ route('admin.customer.toggleBlock', $customer->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @if ($customer->status == 'aktif')
                                                <button type="submit"
                                                    class="flex items-center gap-1.5 bg-amber-100 hover:bg-amber-200 text-amber-800 font-semibold px-3 py-1 rounded-md text-sm transition-colors"
                                            ...
                                            @else
                                                <button type="submit"
                                                    class="flex items-center gap-1.5 bg-green-100 hover:bg-green-200 text-green-800 font-semibold px-3 py-1 rounded-md text-sm transition-colors"
                                        ...
                                        <form action="{{ route('admin.customer.destroy', $customer->id) }}" method="POST"
                                            class="inline"
                                            onsubmit="return confirm('Apakah Anda yakin ingin MENGHAPUS customer ini secara permanen?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="flex items-center gap-1.5 bg-red-100 hover:bg-red-200 text-red-800 font-semibold px-3 py-1 rounded-md text-sm transition-colors"
