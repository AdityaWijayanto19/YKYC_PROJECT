                    ...
                    <div id="order-card-{{ $order->id }}" class="bg-white p-6 rounded-lg shadow-md transition hover:shadow-xl">
                        <div class="flex flex-wrap justify-between items-center gap-4">
                            ...
                            <div id="action-buttons-{{ $order->id }}"
                                class="flex-1 flex flex-col md:flex-row items-center justify-between gap-6">

                                <div class="w-full md:w-auto flex-1 px-4 md:px-0">
                                   @php
                                        $statuses = ['pending', 'on the way', 'diproses', 'ready for pickup', 'completed'];
                                    ...
                                    <div class="relative w-full">
                                        ...
                                        <div class="absolute top-4 left-0 h-1 bg-primary"
                                            style="width: {{ ($currentStatusIndex / (count($statuses) - 1)) * 100 }}%;"></div>
                                        ...
                                        <div class="relative flex justify-between items-start">
                                            @foreach ($statuses as $index => $status)
                                                ...
                                                <div class="flex flex-col items-center text-center w-24">
                                                    <div
                                                        class="flex items-center justify-center w-8 h-8 rounded-full z-10 {{ $isCompleted ? 'bg-primary' : 'bg-gray-300' }} border-2 border-white">
                                                        @if ($isCompleted)
                                ...
                                <div class="flex-shrink-0 flex flex-col sm:flex-row items-center gap-4 mt-8 md:mt-0">
                                    @if ($order->payment_status == 'pending')
                                        <a href="{{ route('customer.order.payment', $order) }}"
                                            class="text-white bg-blue-600 hover:bg-blue-900 font-medium rounded-lg text-sm px-5 py-2.5 text-center flex items-center gap-2">
                                    ...
                                    @if ($order->delivery_method == 'pickup' && $order->payment_status == 'paid' && !in_array($order->status->name, ['completed', 'cancelled', 'dibatalkan']))
